...
class _JobDetailsScreenState extends State<JobDetailsScreen> {
  Future<void> _deleteWorkOrderImage(int idx) async {
    ...
    try {
      ...
      final deleteSuccess = await imageService.deleteImage(imageUrl, SupabaseConfig.storageBucket);
      
      if (!deleteSuccess) {
        Navigator.of(context).pop();
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Failed to delete photo from storage'),
      ...
      final updateSuccess = await provider.updateWorkOrder(widget.workOrder, {
      ...
      if (updateSuccess) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Photo deleted from storage but failed to update work order: ${provider.error ?? "Unknown error"}'),
  ...
  Future<void> _fetchWorkOrder() async {
    try {
      ...
      final workOrderObj = await supabase
          .from('WorkOrders')
          .select('customer_signature, status, image_url')
      ...
      setState(() {
        ...
        if (imageUrlData is List) {
          ...
          _workOrderImageUrls = imageUrlData
              .split(',')
              .map((e) => e.trim())
              .toList();
        } else {
  ...
  Future<void> _processDigitalSignOff() async {
    try {
      ...
      final compressedBytes = img.encodePng(
        decodedImage,
        level: 6,
      ); // level: 0-9, higher is more compressed

      // Save compressed signature as a temporary file
      final tempDir = await getTemporaryDirectory();
      final fileName =
          'signature_${widget.workOrder}_${DateTime.now().millisecondsSinceEpoch}.png';
      final tempFile = File(path.join(tempDir.path, fileName));
      await tempFile.writeAsBytes(compressedBytes);

      // Upload to Supabase Storage (Work Order Photos bucket)
      final imageService = ImageService();
      final imageUrl = await imageService.uploadWorkOrderImage(
        tempFile,
        widget.workOrder,
      );
  ...
  void initState() {
    ...
    if (widget.customerSignature != null &&
        widget.customerSignature!.isNotEmpty) {
      _workOrderImageUrls = widget.customerSignature!
          .split(',')
          .map((e) => e.trim())
          .toList();
    }
    ...
    if (newStatus == 'Completed') {
      if (assignedParts.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Cannot mark as Completed: No assigned parts for this work order.',
            ),
            backgroundColor: Colors.red,
            duration: Duration(seconds: 2),
          ),
      ...
      final unfinishedParts = assignedParts
      ...
      if (unfinishedParts.isNotEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'All assigned parts must be finished before marking job as Completed.',
            ),
            backgroundColor: Colors.red,
            duration: Duration(seconds: 2),
          ),
    ...
    try {
      ...
      if (errorMsg.contains('cameraDelegate')) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Camera is not available on this platform or not configured. Please check your setup.',
            ),
            backgroundColor: Colors.red,
          ),
        );
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error capturing photo: $errorMsg'),
            backgroundColor: Colors.red,
          ),
        );
      }
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (isLoadingParts)
                    ...
                    ...assignedParts.where((part) => part['part_completed_at'] == null).map((
                      part,
                    ) {
                      final partId = part['id'];
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      FutureBuilder<String>(
                                        future:
                                            CurrencyFormatter.formatWithAutoConversion(
                                              partPrice ?? 0,
                                            ),
                      ...
                      builder: (context) {
                        ...
                        return Padding(
                          ...
                          child: FutureBuilder<String>(
                            future: CurrencyFormatter.formatWithAutoConversion(
                              totalSum,
                            ),
                            builder: (context, snapshot) {
                              return Text(
